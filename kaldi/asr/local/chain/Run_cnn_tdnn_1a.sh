...
test_sets="test_data1 test_other"
mfccdir="dump/data"
...
tree_dir=$expdir/chain${nnet3_affix}/tree_sp${tree_affix:+_$tree_affix}
...
if [ $stage -eq 16 ]; then
  ...
  # the lang directory.
  utils/mkgraph.sh --self-loop-scale 1.0 --remove-oov ${data_folder}/lang_test_tgsmall $dir $graph_dir
fi
...
if [ $stage -eq 17 ]; then
  ...
  for decode_set in $test_sets; do
      ...
      steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
          ...
          --online-ivector-dir $expdir/nnet3${nnet3_affix}/ivectors_${decode_set}_hires \
          $graph_dir ${data_folder}/${decode_set}_hires $dir/decode_${decode_set}${decode_iter:+_$decode_iter}_tgsmall || exit 1
      steps/lmrescore.sh --cmd "$decode_cmd" --self-loop-scale 1.0 ${data_folder}/lang_test_{tgsmall,tgmed} \
          ${data_folder}/${decode_set}_hires $dir/decode_${decode_set}${decode_iter:+_$decode_iter}_{tgsmall,tgmed} || exit 1
      steps/lmrescore_const_arpa.sh \
          --cmd "$decode_cmd" ${data_folder}/lang_test_{tgsmall,tglarge} \
          ${data_folder}/${decode_set}_hires $dir/decode_${decode_set}${decode_iter:+_$decode_iter}_{tgsmall,tglarge} || exit 1
      steps/lmrescore_const_arpa.sh \
          --cmd "$decode_cmd" ${data_folder}/lang_test_{tgsmall,fglarge} \
          ${data_folder}/${decode_set}_hires $dir/decode_${decode_set}${decode_iter:+_$decode_iter}_{tgsmall,fglarge} || exit 1
      ) || touch $dir/.error &
  done
  wait
  if [ -f $dir/.error ]; then
...
if $test_online_decoding && [ $stage -eq 18 ]; then
  ...
  for data in test_clean test_other dev_clean dev_other; do
    (
      ...
      steps/online/nnet3/decode.sh \
          ...
          $graph_dir ${data_folder}/${data} ${dir}_online/decode_${data}_tgsmall || exit 1
