...
if [ $stage -le 1 ]; then
  ...
  steps/make_mfcc.sh --cmd "$train_cmd" --nj 50 $data_folder/${train_set}_sp $expdir/make_mfcc/${train_set}_sp $mfccdir/${train_set}_sp || exit 1;
  steps/compute_cmvn_stats.sh $data_folder/${train_set}_sp $expdir/make_mfcc/${train_set}_sp $mfccdir/${train_set}_sp || exit 1;
...
if [ $stage -le 2 ]; then
  ...
  steps/align_fmllr.sh --nj 100 --cmd "$train_cmd" \
    $data_folder/${train_set}_sp $data_folder/lang $gmm_dir $ali_dir || exit 1
fi
...
if [ $stage -le 3 ]; then
  ...
  for datadir in ${train_set}_sp_hires; do
    steps/make_mfcc.sh --nj 70 --mfcc-config conf/mfcc_hires.conf \
      --cmd "$train_cmd" $data_folder/${datadir} $expdir/make_mfcc/${datadir} $mfccdir/${datadir} || exit 1;
...
#  utils/subset_data_dir.sh $data_folder/${train_set}_sp_hires 60000 $data_folder/${train_set}_sp_hires_60k
fi
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/get_pca_transform.sh --cmd "$train_cmd" \
      ...
      --max-utts 10000 --subsample 2 \
       ...
       $expdir/nnet3${nnet3_affix}/pca_transform
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 \
    ...
    $expdir/nnet3${nnet3_affix}/pca_transform $expdir/nnet3${nnet3_affix}/diag_ubm
fi
...
if [ $stage -le 5 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 --num-processes $num_processes \
    $data_folder/${train_set}_sp_hires $expdir/nnet3${nnet3_affix}/diag_ubm $expdir/nnet3${nnet3_affix}/extractor || exit 1;
fi

if [ $stage -le 6 ]; then
  echo "$0: extracting iVectors for training data"
  ivectordir=$expdir/nnet3${nnet3_affix}/ivectors_${train_set}_sp_hires
  ...
  utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
    $data_folder/${train_set}_sp_hires ${ivectordir}/${train_set}_sp_hires_max2

  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 60 \
    ${ivectordir}/${train_set}_sp_hires_max2 $expdir/nnet3${nnet3_affix}/extractor \
    $ivectordir || exit 1;
fi
...
if [ $stage -eq 7 ]; then
  ...
  for data in $test_sets; do
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 10 \
      $data_folder/${data}_hires $expdir/nnet3${nnet3_affix}/extractor \
      $expdir/nnet3${nnet3_affix}/ivectors_${data}_hires || exit 1;
  done
fi
