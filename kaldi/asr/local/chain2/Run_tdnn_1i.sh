...
local/chain/Run_ivector.sh --stage $stage \
	--data_folder $data_folder \
	--expdir $expdir --train-set $train_set \
	--mfccdir $mfccdir \
	--gmm $gmm \
...
lat_dir=$expdir/chain2${nnet3_affix}/${gmm}_${train_set}_sp_lats
dir=$expdir/chain2${nnet3_affix}/tdnn${affix:+_$affix}_sp
train_data_dir=$data_folder/${train_set}_sp_hires
train_ivector_dir=$expdir/nnet3${nnet3_affix}/ivectors_${train_set}_sp_hires
lores_train_data_dir=$data_folder/${train_set}_sp
...
if [ $stage -le 12 ]; then
  ...
  if [ -d $lang ]; then
    if [ $lang/L.fst -nt $data_folder/lang/L.fst ]; then
...
if [ $stage -le 13 ]; then
  ...
  steps/align_fmllr_lats.sh --nj 100 --cmd "$train_cmd" ${lores_train_data_dir} \
    $data_folder/lang $gmm_dir $lat_dir
...
if [ $stage -eq 23 ]; then
  ...
  utils/lang/check_phones_compatible.sh \
    $data_folder/lang_test_tgpr/phones.txt $lang/phones.txt
  utils/mkgraph.sh \
    --self-loop-scale 1.0 $data_folder/lang_test_tgpr \
    $tree_dir $tree_dir/graph_tgpr || exit 1;

  utils/lang/check_phones_compatible.sh \
    $data_folder/lang_test_bd_tgpr/phones.txt $lang/phones.txt
  utils/mkgraph.sh \
    --self-loop-scale 1.0 $data_folder/lang_test_bd_tgpr \
    ...
    (
      ...
      for lmtype in tgpr bd_tgpr; do
        steps/nnet3/decode.sh \
          ...
          $tree_dir/graph_${lmtype} $data_folder/${data}_hires ${dir}/decode_${lmtype}_${data_affix} || exit 1
      done
      steps/lmrescore.sh \
        --self-loop-scale 1.0 \
        --cmd "$decode_cmd" $data_folder/lang_test_{tgpr,tg} \
        $data_folder/${data}_hires ${dir}/decode_{tgpr,tg}_${data_affix} || exit 1
      steps/lmrescore_const_arpa.sh --cmd "$decode_cmd" \
        $data_folder/lang_test_bd_{tgpr,fgconst} \
       $data_folder/${data}_hires ${dir}/decode_${lmtype}_${data_affix}{,_fg} || exit 1
    ) || touch $dir/.error &
  done
  wait
...
if $test_online_decoding && [ $stage -eq 25 ]; then
    ...
    (
      ...
      for lmtype in tgpr bd_tgpr; do
        steps/online/nnet3/decode.sh \
          ...
          $tree_dir/graph_${lmtype} $data_folder/${data} ${dir}_online/decode_${lmtype}_${data_affix} || exit 1
      done
      steps/lmrescore.sh \
        --self-loop-scale 1.0 \
        --cmd "$decode_cmd" $data_folder/lang_test_{tgpr,tg} \
        $data_folder/${data}_hires ${dir}_online/decode_{tgpr,tg}_${data_affix} || exit 1
      steps/lmrescore_const_arpa.sh --cmd "$decode_cmd" \
        $data_folder/lang_test_bd_{tgpr,fgconst} \
       $data_folder/${data}_hires ${dir}_online/decode_${lmtype}_${data_affix}{,_fg} || exit 1
    ) || touch $dir/.error &
  done
  wait
  [ -f $dir/.error ] && echo "$0: there was a problem while decoding" && exit 1
